...
float processNet()
{
    ...
    struct onnx_tensor_t* in = onnx_tensor_search(ctx, "input_1");
    struct onnx_tensor_t* out = onnx_tensor_search(ctx, "dense");
    onnx_tensor_apply(in, (void*)input, sizeof(input));
    //onnx_tensor_dump(in, 1);
    onnx_run(ctx);
    //onnx_tensor_dump(out, 1);
    const float* r = (float*)out->datas;
    return r[0];
}
