...
tc = 755   # target sample count/length
ntc = 1906 # non-target sample count/length [2111 max]
...
if isdir(project):
    ...
    if isfile(project + "/nontargets_x.npy"):
        ...
        for f in files:
            ...
            arr = np.array(arr)
            #print("before:", arr)
            nontargets_x.append(meanNormaliseImage(arr))
    ...
    if isfile(project + "/targets_x.npy"):
        ...
        for f in files:
            ...
            arr = keras.preprocessing.image.img_to_array(img)
            arr = np.array(arr)
            targets_x.append(meanNormaliseImage(arr))
